            ...
            <article>
                <div class="articleWrap">
                    <div class="paperDetailWrap">
                        <div class="titleWrap">
                            <p class="title">알림톡 서비스 충전</p>
                        </div>
                        <div class="detailContent">
                          <div class="tableWrap">
                            <div class="btnWrap searchWrap">
                              <table summary="알림톡 서비스 충전">
                                  <caption class="blind">알림톡 서비스 충전</caption>
                                  <colgroup>
                                    <col style="width:10%;">
                                    <col style="width:10%;">
                                    <col style="width:10%;">
                                  ...
                                  <tbody>
                                    ...
                                    <tr>
                                      ...
                                      <td :colspan="{7: addType==='99'}">
                                        <select v-model="addType" style="width: 100%">
                                          <option disabled value="99">선택하세요.</option>
                                      ...
                                      <!-- 결제타입 무통장 경우 끝-->
                                      <!-- 결제타입 서비스 이거나 무통장이면서 스탠다드(payPlan 값 1) 경우 시작-->
                                      <template v-if="addType === '2' || (addType === '1' && payPlan === 1)">
                                        <th>
                                          상점
                                        </th>
                                        <td>
                                      ...
                                      <!-- 결제타입 서비스 이거나 무통장이면서 스탠다드(payPlan 값 1) 경우 끝-->
                                    </tr>
                                  </tbody>
                              </table>
                                <article>
...
export default {
  ...
  data () {
    return {
      ...
      shopList: {}, // 샵리스트
      addShopId: '0',
      addType: '99', // 결제타입 카드 의 코드값이 0 이므로 99를 default로 사용
      payPlan: 0, // 스탠다드 요금업체여부
      addBankName: '',
      addInstMonth: '0',
      addAmount: '0',
      addCount: '0',
  ...
  methods: {
    ...
    get () {
      if (this.userMenuAuth.retrieve_list) {
        ...
        this.http
          ...
          .catch(e => {
            console.log('TalkServiceCharge.vue : ' + e)
          })
      }
    },
    getConfirmUserInfo () {
    ...
    set () {
      if (this.userMenuAuth.create_data || this.userMenuAuth.modify_data) {
        if (this.validate()) {
          this.isLoading = true
          this.http
            .post('/DataCorrection/AlimTalk/increaseCount',
              {
                ...
                'shopId': this.addType === '2' || (this.addType === '1' && this.payPlan === 1) ? Number(this.addShopId) : 0,
                'installmentMonth': this.addType === '2' || (this.addType === '1' && this.payPlan === 1) ? Number(this.addInstMonth) : 0,
            ...
            .then(result => {
              // console.log(result.data)
              this.isLoading = false
              let msg = '알림톡 서비스 충전처리 되었습니다.'
              if (result.data.resultCode !== 0) {
                msg = '알림톡 서비스 충전처리 실패했습니다.'
              }
    ...
    validate () {
      ...
      if (this.addMemo !== '' && this.addType !== '99' && this.addConfirmUser !== '') {
        rtn = true
      }
      if (this.addType === '1') { // 무통장
        if (this.addBankName === '') {
          rtn = false
        }
        if (this.payPlan === 1 && Number(this.addShopId) === 0) {
          rtn = false
        }
      }
      /**
       * 언제 또 말이 바뀔지 모르니까 일단 주석처리
       */
      // if (this.addType === '2') { // 서비스
      //   if (Number(this.addShopId) === 0) {
      //     rtn = false
      //   }
      // }
