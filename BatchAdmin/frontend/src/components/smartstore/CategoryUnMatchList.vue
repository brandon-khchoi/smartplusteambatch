            ...
            <div class="tableWrap tableList tableGroup tableHover">
              <table summary="미매칭 카테고리">
                ...
                <tbody>
                  ...
                  <tr :key="item.id" :style="[item.category_depth_no === 1 ? { 'background': '#f1f1f1'} : {}]">
                    <td class="textCenter">{{item.first_depth}}</td>
                    <td class="textCenter">{{item.second_depth}}</td>
                    <td class="textCenter">{{item.last_depth}}</td>
                    <td class="textCenter">
                      <select v-if="item.category_depth_no === 1" disabled style="width:90%">
                    ...
                    <td class="textCenter">
                      <template v-if="item.category_depth_no === 1"> <!-- 1뎁스만 있는 경우 버튼 없음-->
                      </template>
                      <template v-else>
                        ...
                        <template v-else>
                          <div :id="idx + '_btn_outer'">
                            ...
                            style="border: 1px solid #e72b4b;color: #e72b4b;">미매칭</button>
                          </div>
                        </template>
                      </template>
                    </td>
