      ...
      <div class="dialog-content">
              ...
              <tr v-for="(item, idx) in batchGroupList" :key="idx">
                ...
                <td>
                  <p
                    class="btn_search"
                    style="width:90%"
                    v-if="item.batch_group_no !== ''"
                    @click="deleteBatchGroup(item.batch_group_no)"
                  >삭제</p>
                </td>
              </tr>
            </tbody>
        ...
        <div class="tableTopWrap">
          <div class="btnWrap" style="float:left">
            <ul>
              <li>
                <p class="btn_add" @click="createNewGroup()">추가</p>
              </li>
            </ul>
          </div>
          <div class="btnWrap">
...
export default {
  ...
  methods: {
    ...
    createNewGroup () {
      ...
      item.display_no = this.batchGroupList.length + 1
      this.batchGroupList.push(item)
    },
    deleteBatchGroup (groupNo) {
      http
